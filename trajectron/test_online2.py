...
def get_maps_for_input(input_dict, scene, hyperparams):
    ...
    heading_angles = None if heading_angles[0] is None else torch.Tensor(heading_angles)
    ...
    return {node: maps[[i]] for i, node in enumerate(nodes_with_maps)}
...
def main():
    ...
    for timestep in range(init_timestep + 1, eval_scene.timesteps):
        ...
        maps = get_maps_for_input(input_dict, eval_scene, hyperparams) if hyperparams['use_map_encoding'] else None
        ...
        if eval_scene.robot is not None and hyperparams['incl_robot_node']:
            ...
            ax.plot(robot_for_plotting[1:, 1], robot_for_plotting[1:, 0],
                    color='r', linewidth=1.0, alpha=1.0)
            circle = plt.Circle((robot_for_plotting[0, 1], robot_for_plotting[0, 0]),
                                0.5, facecolor='r', edgecolor='k', lw=0.5, zorder=3)
